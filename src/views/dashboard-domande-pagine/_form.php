    ...
    <div class="row">
        <div class="col-lg-12 col-sm-12">
            <?php if ($model->sondaggi_id && $model->isNewRecord) : ?>
                <?= $form->field($model, 'sondaggi_id', ['options' => ['style' => 'display: none;']])->hiddenInput()->label(FALSE) ?>
                <?= Html::beginTag('div', ['class' => 'form-group field-sondaggio']) ?>
                <?= Html::tag('h2', $model->getAttributeLabel('sondaggi_id'). ': '. $model->sondaggi['titolo'] ) ?>
            ...
            <?php else : ?>
                ...
                $form->field($model, 'sondaggi_id')->dropDownList(
                    \yii\helpers\ArrayHelper::map(open20\amos\sondaggi\models\Sondaggi::find()->all(), 'id', 'titolo'), ['prompt' => AmosSondaggi::t('amossondaggi', '#select_poll')]
                );
                ?>
            <?php endif; ?>
        </div>
